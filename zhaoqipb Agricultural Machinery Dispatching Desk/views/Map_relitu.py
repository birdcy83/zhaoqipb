...
class Map_relituHandlers(RequestHandler):
    def initialize(self):
        # 这是一个变量，来记录是否重定向
        self.redir = False
    ...
    def get(self, h):  
        ...
        if h == "index":  
            ...
            resp_data['current_user'] = config.options['current_user']
            self.render("map_relitu/index2.html", resp_data=resp_data)
            # 渲染account/index.html模板并传入resp_data参数

        
        if h == "api_relitu":
            # relitu_list = self.application.db.get_all_obj("select * from relitu","relitu")
            relitu_list = self.application.db.get_all_obj("select * from yichao","yichao")
            ...
            if len(relitu_list)==1:
                for k,v in relitu_list:
                    relitu_list[k] = convert_to_json_compatible(v) 
            elif len(relitu_list) > 1:
                for i in range(len(relitu_list)):
                    relitu = relitu_list[i]
                    keys_list = ['id', 'longitude', 'latitude', 'date', 'is_sanitized']  # 使用不同的变量名
                    for k in keys_list:
                        relitu[k] = convert_to_json_compatible(relitu[k])
                    relitu_list[i] = relitu

            # 更新原响应数据中的'data'部分
            resp_data['relitu'] = relitu_list #resp_data['relitu'] 是所有的蚁巢信息
